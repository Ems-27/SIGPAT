 ...
 if(($resultado) AND ($resultado->num_rows!=0)){
	 ...
	 <table class="table table-striped table-hover">
	  ...
	  while($rows_intangivel = mysqli_fetch_assoc($resultado)){
		  ...
		  <th><?php echo "<a href='abater_intangivel_abate.php?id=" . $rows_intangivel['id'] . "'>Abater</a>"; ?></th>
		  </tr>
   ...
   echo '<nav aria-label="paginacao">';
	...
	echo "<span class='page-link'><a href='#' onclick='listar_intangivel_abate(1, $qtd_pag)'>Primeira</a> </span>";
	echo '</li>';
	for ($pag_ant = $pag - $max_links; $pag_ant <= $pag - 1; $pag_ant++) {
		if($pag_ant >= 1){
			echo "<li class='page-item'><a class='page-link' href='#' onclick='listar_intangivel_abate($pag_ant, $qtd_pag)'>$pag_ant </a></li>";
		}
	}
...
for ($pag_dep = $pag + 1; $pag_dep <= $pag + $max_links; $pag_dep++) {
		if($pag_dep <= $quantidade_pg){
			echo "<li class='page-item'><a class='page-link' href='#' onclick='listar_intangivel_abate($pag_dep, $qtd_pag)'>$pag_dep</a></li>";
		}
	}
	echo '<li class="page-item">';
	echo "<span class='page-link'><a href='#' onclick='listar_intangivel_abate($quantidade_pg, $qtd_pag)'>Última</a></span>";
	echo '</li>';
	echo '</ul>';
	echo '</nav>';
