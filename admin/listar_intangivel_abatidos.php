...
$inicio = ($pag * $qtd_pag) - $qtd_pag;
 ...
 $pegar = "SELECT * FROM abate_intangivel ORDER BY id DESC LIMIT $inicio, $qtd_pag";
 ...
 if(($resultado) AND ($resultado->num_rows!=0)){
	...
	<?php
	 ...
	 //Paginação - Somar a quantidade de bens
    $pegar = "SELECT COUNT(id) AS num_resultado FROM abate_intangivel";
    $resultado = mysqli_query($conn, $pegar);
   ...
   echo '<nav aria-label="paginacao">';
	...
	echo "<span class='page-link'><a href='#' onclick='listar_intangivel_abatidos(1, $qtd_pag)'>Primeira</a> </span>";
	echo '</li>';
	for ($pag_ant = $pag - $max_links; $pag_ant <= $pag - 1; $pag_ant++) {
		if($pag_ant >= 1){
			echo "<li class='page-item'><a class='page-link' href='#' onclick='listar_intangivel_abatidos($pag_ant, $qtd_pag)'>$pag_ant </a></li>";
		}
	}
...
for ($pag_dep = $pag + 1; $pag_dep <= $pag + $max_links; $pag_dep++) {
		if($pag_dep <= $quantidade_pg){
			echo "<li class='page-item'><a class='page-link' href='#' onclick='listar_intangivel_abatidos($pag_dep, $qtd_pag)'>$pag_dep</a></li>";
		}
	}
	echo '<li class="page-item">';
	echo "<span class='page-link'><a href='#' onclick='listar_intangivel_abatidos($quantidade_pg, $qtd_pag)'>Última</a></span>";
	echo '</li>';
	echo '</ul>';
	echo '</nav>';
